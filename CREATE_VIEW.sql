...
GREATEST(B.WRITE_DATE, IFNULL(MAX(R.WRITE_DATE), B.WRITE_DATE)) AS CHANGE_DATE,
IFNULL(S.ARTICLE_STAR, 0) AS ARTICLE_STAR,
IFNULL(R.TOTAL_REPLY, 0) AS TOTAL_REPLY,
P.MEMBER_POINT
FROM BOARD B
LEFT OUTER JOIN MEMBER M ON B.MEMBER_NO = M.MEMBER_NO
LEFT JOIN MEMBER_POINTVIEW P ON B.MEMBER_NO = P.MEMBER_NO
LEFT JOIN (SELECT COUNT(REPLY_NO) AS TOTAL_REPLY, MAX(WRITE_DATE) AS WRITE_DATE, ARTICLE_NO FROM REPLY GROUP BY ARTICLE_NO) R ON B.ARTICLE_NO = R.ARTICLE_NO
LEFT JOIN (SELECT COUNT(*) AS ARTICLE_STAR, ARTICLE_NO FROM STAR GROUP BY ARTICLE_NO) S ON B.ARTICLE_NO = S.ARTICLE_NO
...
SELECT MEMBER_NO, MEMBER_ID, MEMBER_PWD, IFNULL(M.MEMBER_NICK, MEMBER_ID) AS MEMBER_NICK, LEVEL_NAME, ABOUT_ME, MEMBER_STATUS, TAG_ID,
		JOIN_DATE, LOGIN_DATE, MEMBER_POINT, ARTICLE_NO, REPLY_DATE, 
		CASE WHEN MEMBER_POINT = 0 THEN 100
		ELSE CAST(RANK() OVER(ORDER BY MEMBER_POINT DESC) / (SELECT COUNT(*) FROM MEMBER WHERE MEMBER_STATUS='Y') * 100 AS signed integer)
		END AS RANKING
FROM MEMBER M
LEFT OUTER JOIN (
SELECT MEMBER_NO, ARTICLE_NO, REPLY_DATE
FROM BOARDVIEW
...
SELECT REPLY_NO, MEMBER_NO, MEMBER_NICK, MEMBER_POINT, 
	    ...
	    R.ARTICLE_NO, REPLY_LIKE, CONTENT, WRITE_DATE, MODIFY_DATE, REPLY_STATUS, REF_REPLY_NO 
FROM REPLY R
